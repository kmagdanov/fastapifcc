...
def get_posts(limit: int = 10, skip: int = 0, search: str = '', db: Session = Depends(get_db), 
...
def update_post(id: int, new_post_data: schemas.PostCreate, db: Session = Depends(get_db), 
                current_user: dict = Depends(oauth2.get_current_user)):
    ...
    post_query.update(new_post_data.model_dump(), synchronize_session=False)
    db.commit()
